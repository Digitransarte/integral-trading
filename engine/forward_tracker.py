...
class Position:
    ...
    trailing_pct: float = 0.0      # % trailing stop (0 = fixo)
    highest_price: float = 0.0     # máximo favorável atingido
...
class ForwardTracker:
    ...
    def _check_exit(self, pos: Position, price: float, days_held: int) -> str:
        """Decide se a posição deve ser fechada — suporta LONG/SHORT e trailing."""
        import json as _json
        try:
            meta = _json.loads(pos.metadata or "{}")
        except Exception:
            meta = {}

        direction    = meta.get("direction", "LONG")
        trailing_pct = float(meta.get("trailing_pct", 0.0))
        ...
        if direction == "SHORT":
            ...
            if trailing_pct > 0:
                best = float(meta.get("best_price", pos.entry_price))
                if price < best:
                    best = price
                    meta["best_price"] = best
                    # actualizar metadata (best_price evolui)
                trailing_exit = best * (1 + trailing_pct / 100)
                if price >= trailing_exit and best < pos.entry_price:
                    return "trailing"
        else:
            # LONG: stop abaixo, target acima
            if price <= pos.stop_price:
                return "stop"
            if price >= pos.target_1:
                return "target_1"
    ...
    def _update_metadata(self, pos_id: int, meta: dict):
        import json as _json
        with get_conn() as conn:
            conn.execute(
                "UPDATE positions SET metadata = ? WHERE id = ?",
                (_json.dumps(meta), pos_id)
            )
