"""Integral Trading — Episodic Pivot Strategy v2.2
Critérios baseados no knowledge JSON (knowledge/ep_strategy.json).
v2.2: Volume restrito a 5-10x (sweet spot validado), stop 6%, target 20%.
"""
import json
import os
...
class EpisodicPivotStrategy(BaseStrategy):
    ...
    MIN_GAP_PCT      = 8.0    # Pradeep usa 8%
    MIN_VOLUME_RATIO = 5.0    # Sweet spot validado em backtest: 5-10x
    MAX_VOLUME_RATIO = 10.0   # Cap — acima de 10x tende a reverter (PF 0.23 em backtest)
    MIN_PRICE        = 1.0
    MIN_VOLUME_ABS   = 300000

    default_stop_loss_pct = 0.06   # Reduzido de 8% para 6% — cortar perdas mais cedo
    ...
    def generate_signal(self, ticker: str, df: pd.DataFrame) -> Optional[Signal]:
        ...
        # Stop: mínima dos 2 dias anteriores (Pradeep) ou 6% — o maior dos dois
        ...
        stop         = max(stop_pradeep, stop_pct)
        ...
        return Signal(
            ...
            entry_price=price,
            stop_loss=stop,
            target_1=price * 1.20,   # Aumentado de 15% para 20% — deixar vencedores correr
            target_2=price * 1.40,   # Aumentado de 30% para 40%
            score=score,
            catalyst="Gap + Volume",
            notes=(
