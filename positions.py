...
def render():
    ...
    with tab3:
        ...
        with col1:
            ticker    = st.text_input("Ticker", placeholder="GC=F ou HIMS").upper()
            strategy  = st.selectbox("Estrategia", ["NCI", "EP", "CANSLIM", "Manual"])
            catalyst  = st.text_input("Catalisador", placeholder="NCI setup A+ | KL $4508")
            direction = st.radio("Direcção", ["LONG", "SHORT"], horizontal=True)

        # ── Botão Preencher com NCI ───────────────────────────────────────────
        if ticker and strategy == "NCI":
            if st.button("🔍 Preencher com NCI"):
                with st.spinner("A analisar " + ticker + "..."):
                    try:
                        from engine.nci_analyzer import NCIAnalyzer
                        sig = NCIAnalyzer(DataFeed(polygon_key=POLYGON_API_KEY)).analyze(ticker)
                        if sig.direction != "NONE" and sig.entry_price > 0:
                            st.session_state["nci_prefill"] = {
                                ...
                                "catalyst": sig.setup_description[:80],
                                "score":    sig.confluence_score,
                            }
                            st.success(sig.direction + " | Score " + str(sig.confluence_score) +
                                       "/100 | " + sig.setup_quality +
                                       (" | Activo ✅" if sig.setup_active else " | Aguardar BOS ⏳"))
                        else:
                            st.warning("Sem sinal NCI para " + ticker + ".")
                            st.session_state.pop("nci_prefill", None)
                    except Exception as e:
                        st.error("Erro: " + str(e))

        prefill          = st.session_state.get("nci_prefill", {})
        default_entry    = float(prefill.get("entry",    10.00))
        default_stop     = float(prefill.get("stop",      9.00))
        default_target_1 = float(prefill.get("target_1", 11.50))
        default_target_2 = float(prefill.get("target_2", 13.00))
        default_score    = float(prefill.get("score", 0))

        with st.form("open_position_form"):
            col_f1, col_f2 = st.columns(2)
            with col_f1:
                entry_price = st.number_input("Preco de Entrada ($)",
                    min_value=0.01, value=default_entry, step=0.01, format="%.2f")
                stop_price  = st.number_input("Stop Loss ($)",
                    min_value=0.01, value=default_stop, step=0.01, format="%.2f")
            with col_f2:
                target_1 = st.number_input("Target 1 ($)",
                    min_value=0.01, value=default_target_1, step=0.01, format="%.2f")
                target_2 = st.number_input("Target 2 ($)",
                    min_value=0.01, value=default_target_2, step=0.01, format="%.2f")

            trailing_pct = st.slider("Trailing Stop (%)",
                min_value=0.0, max_value=5.0, value=2.0, step=0.5,
                help="0 = stop fixo. 2.0 = validado para ouro/petróleo.")
            ...
            if entry_price > 0 and stop_price > 0 and target_1 > 0:
                ...
                st.caption("Risco: -{:.1f}%  |  Reward: +{:.1f}%  |  R/R: {:.2f}".format(risk, reward, rr))

            submitted = st.form_submit_button("Abrir Posição", use_container_width=True, type="primary")

        if submitted:
            if not ticker:
                st.error("Introduz um ticker.")
            elif direction == "LONG" and stop_price >= entry_price:
                st.error("Stop loss deve ser inferior ao preço de entrada (LONG).")
            elif direction == "SHORT" and stop_price <= entry_price:
                st.error("Stop loss deve ser superior ao preço de entrada (SHORT).")
            elif direction == "LONG" and target_1 <= entry_price:
                st.error("Target 1 deve ser superior ao preço de entrada (LONG).")
            elif direction == "SHORT" and target_1 >= entry_price:
                st.error("Target 1 deve ser inferior ao preço de entrada (SHORT).")
            else:
                pos_id = tracker.open_position(
                    ticker=ticker, strategy=strategy,
                    entry_price=entry_price, stop_price=stop_price,
                    target_1=target_1, target_2=target_2,
                    catalyst=prefill.get("catalyst", catalyst),
                    direction=direction, trailing_pct=trailing_pct,
                    score=default_score,
                )
                st.session_state.pop("nci_prefill", None)
                st.success("Posição aberta: " + ticker + " (" + direction + ") @ $" +
                           str(round(entry_price, 2)) + " (ID #" + str(pos_id) + ")")
                st.rerun()
